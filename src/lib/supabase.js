...
export const SUPABASE_ANON_KEY = 'sb_publishable_2dG-vOprM7FzMmqRA7Ps6A_5zAaPS_K'
...
export async function adminFetchAllPosts() {
  try {
    ...
    console.error('adminFetchAllPosts error:', err)
    return []
  }
}
...
export function getMockPosts(filter = {}) {
  const posts = [
    { id:'1', slug:'fuel-subsidy-reform-impact', title:"How the Fuel Subsidy Reform is Unlocking Nigeria's Economic Potential", excerpt:"President Tinubu's bold decision to remove fuel subsidies has freed over ₦11 trillion annually.", author:'RTIFN Media Team', date:'2026-01-10', category:'Economic Reforms', featuredImage:null, published:true, htmlContent:'<p>The removal of the fuel subsidy was one of the most consequential economic decisions in recent history...</p>' },
    { id:'2', slug:'rtifn-osun-ward-sensitization', title:'RTIFN Osun Launches Ward-by-Ward Sensitization Campaign', excerpt:"RTIFN has begun an intensive grassroots mobilization drive across all 30 LGAs.", author:'RTIFN Media Team', date:'2026-01-15', category:'RTIFN Activities', featuredImage:null, published:true, htmlContent:'<p>RTIFN Osun State Chapter has commenced its most ambitious grassroots engagement drive to date...</p>' },
    { id:'3', slug:'faac-allocation-increase', title:'Record FAAC Allocations Under Tinubu', excerpt:'Monthly federation account distributions to states have surged significantly.', author:'RTIFN Policy Desk', date:'2026-01-20', category:'Tinubu Achievements', featuredImage:null, published:true, htmlContent:'<p>One of the most impactful outcomes of President Tinubu\'s reforms is the dramatic increase in FAAC allocations...</p>' },
  ]
  let result = [...posts]
